        ...
        <div class="w-3/4 mx-auto pt-44 md:pt-32">
            <div class="flex flex-col items-center justify-center w-full gap-8 px-5 py-20 mt-2 mb-6 bg-white rounded-lg shadow md:flex-row">
                <div class="flex justify-center w-full overflow-hidden md:w-7/12">
                    <img class="object-center w-full transition transform h-72 duration-1500 hover:scale-125" src="https://dummyimage.com/1200x800/000000/ffffff&text=Product{{ $itemId }}" alt="{{ $product->title }} - {{ config('app.name') }}">
                </div>
                <div class="flex flex-col justify-center w-full">
                    ...
                    <div class="flex items-center mt-2" wire:loading.class='opacity-10' wire:target='addToCart,removeFromCart'>
                        ...
                        <div class="text-center ml-9">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 cursor-pointer" fill="{{ $inWL ? 'red' : 'none' }}" viewBox="0 0 24 24" stroke="{{ $inWL ? 'red' : 'currentColor' }}" wire:click='{{ $inWL ? 'removeFromWL' : 'addToWL' }}'>
